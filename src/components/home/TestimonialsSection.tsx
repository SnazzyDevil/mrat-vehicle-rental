...
const TestimonialsSection = () => {
  ...
  const filteredTestimonials = filter === 'all' 
    ? testimonials 
    : testimonials.filter(testimonial => testimonial.type === filter);

  return (
    <section className="section-padding bg-gradient-to-br from-primary-blue to-primary-light text-white">
        ...
        <div className="text-center mb-12">
          ...
          <div className="flex justify-center space-x-4 mb-8">
            <button 
              ...
              className={`px-6 py-2 rounded-full transition-colors ${filter === 'all' ? 'bg-white text-primary-blue' : 'bg-transparent border border-white'}`}
            >
              All
            </button>
            <button 
              onClick={() => setFilter('business')}
              className={`px-6 py-2 rounded-full transition-colors ${filter === 'business' ? 'bg-white text-primary-blue' : 'bg-transparent border border-white'}`}
            >
              Business
            </button>
            <button 
              onClick={() => setFilter('personal')}
              className={`px-6 py-2 rounded-full transition-colors ${filter === 'personal' ? 'bg-white text-primary-blue' : 'bg-transparent border border-white'}`}
            >
              Personal
            </button>
          </div>
