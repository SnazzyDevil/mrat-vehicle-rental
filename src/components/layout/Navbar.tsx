...
const Navbar = () => {
  ...
  const navItems = [
    ...
    { 
      ...
      dropdown: true,
      items: [
        { title: 'Business Rentals', path: '/business-rentals' },
        { title: 'Personal Rentals', path: '/personal-rentals' },
      ]
    },
  ...
  return (
    <nav className={`fixed top-0 w-full z-50 ${isScrolled ? 'bg-white shadow-md' : 'bg-transparent'} transition-all duration-300`}>
      <div className="container mx-auto px-4 md:px-6">
              ...
              item.dropdown ? (
                <div key={item.title} className="relative group">
                  <button 
                    ...
                    className="px-3 py-2 flex items-center text-gray-700 hover:text-primary-blue"
                  >
                    {item.title}
                ...
                <Link
                  key={item.title}
                  to={item.path}
                  className="px-3 py-2 text-gray-700 hover:text-primary-blue"
                >
                  {item.title}
                </Link>
              )
        ...
        {/* Mobile Menu */}
        {isOpen && (
          <div className="md:hidden bg-white shadow-lg absolute top-20 left-0 right-0 z-50 animate-fade-in">
              ...
              {navItems.map((item) => 
                item.dropdown ? (
                  <div key={item.title}>
                    <button 
                      onClick={() => toggleDropdown(item.title)}
                      className="block w-full text-left px-3 py-2 text-base font-medium text-gray-700 hover:text-primary-blue hover:bg-gray-50 rounded-md"
                    >
