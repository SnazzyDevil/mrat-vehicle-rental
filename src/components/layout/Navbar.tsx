...
const Navbar = ({
  ...
  return <nav className={`fixed top-0 w-full z-50 ${isScrolled ? 'bg-white shadow-md' : 'bg-transparent'} transition-all duration-300`}>
      <div className="container md:px-6 px-[21px] py-0 mx-0 my-0 bg-[#601112]">
        <div className="flex items-center justify-between h-20">
          ...
          <div className="hidden md:flex items-center space-x-1">
            {navItems.map(item => item.dropdown ? <div key={item.title} className="relative group">
                  <button onClick={() => toggleDropdown(item.title)} className="px-3 py-2 flex items-center text-accent-white hover:text-primary-blue">
                    {item.title}
