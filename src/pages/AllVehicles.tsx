...
const AllVehicles = () => {
  ...
  const groupContainsType = (groupId: string, type: string): boolean => {
    ...
    switch(type) {
      case 'car': return carGroups.includes(groupId);
      case 'van': return vanGroups.includes(groupId);
      case 'truck': return truckGroups.includes(groupId);
      default: return true;
    }
  };
          ...
          <div className="container mx-auto px-4 md:px-6">
            <div className="flex flex-wrap justify-center gap-2 mb-6">
              ...
              <Button
                ...
                className={selectedType === "car" ? "bg-primary-maroon hover:bg-primary-light" : "text-primary-maroon border-primary-maroon hover:bg-primary-maroon hover:text-white"}
              >
                Cars
              </Button>

              <Button
                variant={selectedType === "van" ? "default" : "outline"}
                onClick={() => setSelectedType("van")}
                className={selectedType === "van" ? "bg-primary-maroon hover:bg-primary-light" : "text-primary-maroon border-primary-maroon hover:bg-primary-maroon hover:text-white"}
              >
                Vans
              </Button>
              ...
              <Button
                ...
                className={selectedType === "truck" ? "bg-primary-maroon hover:bg-primary-light" : "text-primary-maroon border-primary-maroon hover:bg-primary-maroon hover:text-white"}
              >
                Trucks
              </Button>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
              {filteredVehicles.map(group => {
                ...
                if (group.id === 'D' && selectedType === 'van') {
                  return (
                    <Card key={`${group.id}-van`} className="overflow-hidden transition-all duration-300 hover:shadow-lg hover:-translate-y-1">
                      <div className="h-60 overflow-hidden">
                        <img
                          src="https://mrat.creativortexstudio.co.za/wp-content/uploads/2025/05/Toyota-Hilux-Single-Cab-Canopy.png"
                          alt="Group D - 1 ton single cab van"
                ...
                if (group.id === 'H' && selectedType === 'van') {
                  return (
                    <Card key={`${group.id}-van`} className="overflow-hidden transition-all duration-300 hover:shadow-lg hover:-translate-y-1">
                      <div className="h-60 overflow-hidden">
                        <img
                          src="https://mrat.creativortexstudio.co.za/wp-content/uploads/2025/05/Fortuner.png"
                          alt="Group H - Family SUV"
                          className="w-full h-full object-cover transform transition-transform hover:scale-105 duration-500"
                        />
                        <div className="absolute top-4 left-4 bg-primary-maroon text-white px-3 py-1 rounded-md">
                          Group {group.id}
                ...
                if (group.id === 'I' && selectedType === 'truck') {
                  return (
                    <Card key={`${group.id}-truck`} className="overflow-hidden transition-all duration-300 hover:shadow-lg hover:-translate-y-1">
                      <div className="h-60 overflow-hidden">
                        <img
                          src="https://mrat.creativortexstudio.co.za/wp-content/uploads/2025/05/Fridge.png"
                          alt="Group I - 5 ton fridge truck"
                          className="w-full h-full object-cover transform transition-transform hover:scale-105 duration-500"
                        />
                        <div className="absolute top-4 left-4 bg-primary-maroon text-white px-3 py-1 rounded-md">
                          Group {group.id}
                ...
                if (group.id === 'J' && selectedType === 'truck') {
                  return (
                    <Card key={`${group.id}-truck`} className="overflow-hidden transition-all duration-300 hover:shadow-lg hover:-translate-y-1">
                      <div className="h-60 overflow-hidden">
                        <img
                          src="https://mrat.creativortexstudio.co.za/wp-content/uploads/2025/05/Isuzu-800.png"
                          alt="Group J - 8 ton Fridge Truck"
                          className="w-full h-full object-cover transform transition-transform hover:scale-105 duration-500"
                        />
                        <div className="absolute top-4 left-4 bg-primary-maroon text-white px-3 py-1 rounded-md">
                          Group {group.id}
                  ...
                  <Card key={group.id} className="overflow-hidden transition-all duration-300 hover:shadow-lg hover:-translate-y-1">
                    ...
                    <CardContent className="p-6">
                      ...
                      <div className="flex gap-2 mb-4">
                        ...
                        {groupContainsType(group.id, 'car') &&
                          group.id !== 'D' &&
                          !(group.id === 'J' && selectedType === 'truck') && (
                          <span className="bg-blue-100 text-blue-800 text-xs font-medium px-2.5 py-0.5 rounded">Car</span>
                        )}
                        {/* Special case for Group D car badge */}
                        {(groupContainsType(group.id, 'car') && group.id === 'D' && selectedType !== 'van') && (
                          <span className="bg-blue-100 text-blue-800 text-xs font-medium px-2.5 py-0.5 rounded">Car</span>
                        )}
                        {/* Van badge - show for all van groups except when viewed as special cases */}
                        {groupContainsType(group.id, 'van') &&
                          group.id !== 'D' &&
                          !(group.id === 'I' && selectedType === 'truck') &&
                          !(group.id === 'H' && selectedType === 'van') && (
                          <span className="bg-green-100 text-green-800 text-xs font-medium px-2.5 py-0.5 rounded">Van</span>
                        )}
                        {/* Truck badge - show for all truck groups except when viewed as special cases */}
                        {groupContainsType(group.id, 'truck') &&
                          !(group.id === 'J' && selectedType === 'truck') &&
                          !(group.id === 'J' && selectedType === 'car') &&
                          !(group.id === 'I' && selectedType === 'truck') &&
                          !(group.id === 'H' && selectedType === 'van') && (
                          <span className="bg-amber-100 text-amber-800 text-xs font-medium px-2.5 py-0.5 rounded">Truck</span>
                        )}
